...
async function readProduct(body, response) {
    ...
    // Revisar validaciones de errores
    /* if (result == null) { // Undefined here
        const message = { message: "El producto con el sku " + body.sku + " no existe" };
...
async function createProduct(sku, name_product, price, description_product, url_image, response) {
    ...
    const result = await connection.query(
        "INSERT INTO `products` (`sku`, `name_product`, `price`, `description_product`, `url_image`) VALUES (?,?,?,?,?)",
        [sku, name_product, price, description_product, url_image]
    );
    return response.json(result);
}
// solucionar
async function updateProduct(sku, name_product, price, description_product, url_image, response) {
    ...
    const connection = await getConnection();
    const result = await connection.query("UPDATE products SET `name_product`= ?,`price`=?,`description_product`=?,`url_image`=? WHERE sku= ?", [
        name_product,
        price,
        description_product,
        url_image,
